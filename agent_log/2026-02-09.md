...
## 2026-03-19
- Updated `searchTokenInfo` docs to use `query` (required), `quote_currency` (optional), and `chain_index` (optional).
- Added explicit URL-call guidance: `token/v2/analysis?query=BTC&quote_currency=USDT&chain_index=1`.
- Rebuilt/exported docs so both `content/` and `gh-pages-dist/` pages reflect the same contract.
- Refined the URL-call description to avoid hardcoded query values and state that parameters must be selected by the agent from user intent.
- Restructured the `searchTokenInfo` page with a dedicated `Endpoint` section and `Use Rules` that require calling the remote staging endpoint with agent-determined parameters.
- Updated `projectTool` docs with a dedicated `Endpoint` section and `Use Rules` requiring calls to the remote `/project/analysis` endpoint with non-hardcoded, intent-driven parameters.
- Restructured site information architecture for progressive disclosure with a clearer home page and explicit shallow-depth navigation rules.
- Incorporated `fast_agent` prompt guidance into docs: tool-first behavior, documentation-driven flow, no silent failure, confidentiality policy, memory read/write rules, and canonical `BSC` naming.
- Reworked `tool/index.md` into a stronger execution router and added endpoint-critical routing notes for token/project analysis tools.
- Updated README architecture notes to reflect the new home-first, low-depth design model.
- Added explicit progressive-disclosure control rules (fetch budget, stop conditions, and layer-specific access gating) to reduce over-fetching during agent runs.
- Hardened server runtime for concurrency: indexed routes in memory, cached markdown file buffers, enabled global compression, and tuned Fastify timeouts/keepalive limits.
- Added graceful shutdown handlers (`SIGINT`/`SIGTERM`) for safer restarts under load.
- Added viewer-side reliability controls: markdown fetch timeout, bounded TTL cache, in-flight render coalescing, and stale-cache fallback on transient upstream failures.
- Updated server README with reliability env knobs and corrected TypeScript-typed `marked` options for current library definitions.
- Added explicit client-abort error handling for `premature close` / reset scenarios so disconnects are treated as expected request cancellations instead of high-severity runtime failures.
- Updated markdown response behavior to always return `200` with body (no `304` short-circuit) so direct browser visits like `/tool/` always render content predictably.
- Removed response compression from the markdown server path to avoid repeated `stream closed prematurely` / `premature close` log noise during normal browser navigation.
- Updated redirect usage to Fastify's non-deprecated signature (`reply.redirect(url, code)`).
- Restructured SOUL/RUNTIME information architecture: promoted core guidance directly to `/soul/`, moved runtime rules to top-level `/runtime/`, updated cross-links, and removed the `skill/` directory so it is hidden from home-page subdirectories.
- Split output-style guidance into a dedicated top-level `/output/` directory and moved response format, writing/citation rules, and token output constraints out of `/runtime/`.
- Updated home progressive-disclosure instructions to explicitly allow multi-page exploration when needed instead of implying a single-subpage limit.
- Expanded `/runtime/` with explicit tool workflows/tool combinations by query type, dependency sequencing rules, and data-reliability hierarchy guidance.
- Reorganized instruction ownership by directory: home is now a router, `soul` focuses on identity/safety/truth, `runtime` carries execution policy/workflows, `output` carries response-style policy, and `tool` no longer implies a one-subpage traversal limit.
